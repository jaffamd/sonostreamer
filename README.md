...
Move the `configure.sh` file from the boot folder to the root folder:
~~~
sudo mv /boot/configure.sh ./
~~~
. Run it with:
~~~~
...
Once the access point configuration is done, it will tell you to restart. Don't (you'll probably lose your connection to the box; don't worry, this step comes later). Instead, run the following code to allow the networks to load properly on boot:
~~~
...
At this point, you should transfer the files from the folder "PHP Files for Pi" to the `/var/www/html` folder on the Pi using the `scp` command (`scp` is the [secure file transfer protocol over SSH](https://research.csc.fi/csc-guide-copying-files-from-linux-and-mac-osx-machines-with-scp)).
...
Finally, change the permissions and ownership of the files with the following commands:
~~~
sudo usermod -a -G video www-data
